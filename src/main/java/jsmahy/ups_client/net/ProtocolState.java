...
public enum ProtocolState {
	...
	public PacketOut getPacketOut(int packetId, String data) throws InvalidPacketFormatException {
		...
		try {
			...
			L.fatal(String.format("Packet %s does not implement '%s' method!", clazz.getSimpleName(),
					DESERIALIZE_METHOD), e);
			Platform.exit();
			return null;
		}
		...
		try {
			...
			L.fatal(String.format("An exception occurred when instantiating %s's constructor!", clazz.getSimpleName()),
					e);
			Platform.exit();
			return null;
		}
